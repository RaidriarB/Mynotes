...
class Challenges:
    ...
    def challenge_1(self, answer): # "Hello"
        if answer == "Hello":
            return True

    def challenge_2(self, answer):# (1,1,[1],[1])
        a, b, c, d = answer
        if a == b and a is b and c == d and c is not d:
            return True

    def challenge_3(self, answer):#??? # "1"
        if answer in answer == answer:
            return True

    def challenge_4(self, answer):# [1],1
        a1, b1 = answer
        a2, b2 = answer
    ...
    def challenge_5(self, answer): # [3,2,1]
        r = reversed([1, 2, 3])
        if list(r) == list(r) + answer:
            return True

    def challenge_6(self, answer):#?? # {1,2},{1,3}
        a, b = answer
        if max(a, b) != max(b, a):
            return True

    def challenge_7(self, answer):# 2,[1],[2]
        a, b, c = answer
        for x in a, b, c:
    ...
    def challenge_8(self, answer):#重复因子为负数将输出空序列 #-1,-1,[1]
        a, b, c = answer
        for x in a, b, c:
            if isinstance(x, float) or isinstance(x, complex):
                return False
    ...
    def challenge_9(self, answer):# -1,2
        a, b = answer
        for x in a, b:
            if isinstance(x, float) or isinstance(x, complex):
                return False
    ...
    def challenge_10(self, answer):# "1",""
        a, b = answer
        if a and a.count(b) > len(a):
            return True

    def challenge_11(self, answer):# * 运算符会进行一次解包  #[[1]]
        if max(answer) != max(*answer):#max 可以接受多个数，也可以接受iterable
            return True

    def challenge_12(self, answer):#{2},{1,2}
        a, b = answer
        if a < b and all(x > y for x, y in zip(a, b)):
            return True

    def challenge_13(self, answer):# {1,2},{2}
        a, b = answer
        if b and not (a ^ b) - a:
            return True
    ...
    def challenge_15(self, answer):# {2,3},[{1,2},{2,3}]
        item, l = answer
    ...
    def challenge_17(self, answer):# 1,{0:1}
        item, l = answer
        ...
        if answer.isdecimal():
            if len(answer) < 5:
                if sum(ord(c) - ord("0") for c in answer) == 23333:
                    return True
        print(sum(ord(c) - ord("0") for c in answer))
